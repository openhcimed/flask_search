...
raw_image = np.load('./weights/raw_new_50.npy')
#[:, :, :, 0]  # Candidates, i.e. DeepLesion test

# images
emb_image = np.load('./weights/emb_image.npy')
# Pre-computed embeddings of candidates
label = np.load('./weights/label.npy')          # Label

# Load model weights
vae = LesionVAE.load_from_checkpoint('./weights/vae.ckpt')
#vae = LesionSimCLR.load_from_checkpoint('./weights/vae.ckpt', gpus=1, num_samples=1, batch_size=1,
# dataset='lesion')
# vae.eval()
vae.eval()
...
def get_image_embedding(img):
    '''Get image embedding with VAE and vae'''
...
#frangi(emb_image, scale_range=(2, 3), scale_step=1, beta=6,
...
def index():
    if request.method == 'POST':
        ...
        # query = query.numpy()
        tempquery = query
        #query = frangi(query, sigmas=(2,3), scale_step=1, beta=6,
                                             #gamma=0.00002, black_ridges=False)
        query_emb_vae = get_image_embedding(query)
